...
def encode_decode_handler():
    ...
    if request.method == 'POST':
        ...
        if is_ascii(input_text):  # Proceed only if the input is not empty
            encoded_output = encode(input_text)  # Encode the input
            decoded_output = decode(encoded_output)  # Decode for verification
        else:
            error_text = "Input Text is not ASCII-compatible."
